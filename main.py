...
class CarClassifier:
    ...
    bmw_label_map = {0: 'BMW 1 Series M 2011', 1: 'BMW 1 Series convertible 2011', 2: 'BMW 1 Series couple 2011',
                     ...
                     9: 'BMW 2 Series Active Tourer 2014', 10: 'BMW 2 Series Active Tourer 2015',
                     11: 'BMW 3 Series 2008',
                     ...
                     19: 'BMW 3 Series GT 2014', 20: 'BMW 3 Series convertible 2008',
                     21: 'BMW 3 Series convertible 2010',
                     ...
                     119: 'BMW X5 M 2016', 120: 'BMW X6 2010', 121: 'BMW X6 2011', 122: 'BMW X6 2012',
                     123: 'BMW X6 2013',
    ...
    def __init__(self, models_path):
        if self.make_model is None:
            self.make_model = keras.models.load_model(models_path + "/make_model.h5")

        if self.bmw_model is None:
            self.bmw_model = keras.models.load_model(models_path + "/bmw_model.h5")

    def get_model_pred(self, model, img, label_map):
    ...
    def predict(self, img_path, actual_label, visualize=True):
        # Run YOLOv3 to get cropped images of cars
        img, cropped_imgs = get_bounding_images_p(img_path)
        ...
        pred_make = self.get_model_pred(self.make_model, car_img_p, self.make_label_map)
        pred_model = ""

        # If the predicted label is BMW then us the BMW model
        if pred_make == "BMW":
            pred_model = self.get_model_pred(self.bmw_model, car_img_p, self.bmw_label_map)
        else:
            return None
        ...
        accuracy = sift_authenticator(cropped_imgs[0][0], pred_make, pred_model)
        ...
        cv2.putText(img, "CONFIDENCE: " + str(accuracy) + "%", (x + 10, y + h - 10),
...
def predict_on_dataframe(csv_path):
    car_classifier = CarClassifier("models")
    test_df = pd.read_csv(csv_path)  # "data/bmw_test_data.csv"
    ...
    for index, row in test_df.iterrows():
        ...
        if row["label"] is not None and row["filename"] is not None:
            try:
                ...
                label = row["label"]
                car_classifier.predict(img_path, label)
...
def predict_on_single(img_path, label):
    car_classifier = CarClassifier("models")
    try:
        car_classifier.predict(img_path, label)
    except Exception as inst:
...
if __name__ == "__main__":
    print("******************************************")
    print("*                                        *")
    print("*  Systematic Car Classification System  *")
    print("*                                        *")
    print("******************************************")
    ...
    if image_path is not None and image_path.strip() != "":
        predict_on_single(image_path, actual_label)
    elif csv_path is not None and csv_path.strip() != "":
        predict_on_dataframe(csv_path)
    else:
        raise Exception("No image_path or csv_path was given. "
                        "Use --help to check for the required arguments.")
